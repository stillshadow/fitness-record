(() => {
  ...
  function loadExisting(date=editDate){
    ...
    const groups=[];
    rows.forEach(r=>{
      let g=groups.find(x=>x.exerciseId===r.exerciseId);
      if(!g){
        const ex=(db.exercises||[]).find(x=>x.id===r.exerciseId)||{id:r.exerciseId,name:r.exerciseName||'未知动作',group:''};
        g={ex,rows:[]}; groups.push(g);
      }
      g.rows.push(r);
    });
    drafts=groups.map(g=>draftForExercise(g.ex,g.rows.length,g.rows.sort((a,b)=>(+a.setIndex||0)-(+b.setIndex||0))));
  }
  ...
  function ensureUI(){
    ...
    const style=document.createElement('style'); style.id='batchTrainingStyle'; style.textContent=`
      ...
      .batch-list{display:grid;gap:10px}.batch-ex{border:1px solid var(--line);background:var(--panel2);border-radius:16px;padding:11px}.batch-ex-head{display:flex;align-items:start;justify-content:space-between;gap:10px;margin-bottom:9px}.batch-ex-head b{font-size:14px}.batch-ex-head small{display:block;color:var(--muted);font-size:10px;margin-top:2px}.batch-remove{border:0;background:transparent;color:var(--bad);font-size:12px;padding:3px 2px}
      ...
      .batch-add-ex{display:grid;grid-template-columns:minmax(0,1fr) auto;gap:8px;margin-top:10px}.batch-empty{border:1px dashed var(--line);border-radius:14px;padding:22px 12px;text-align:center;color:var(--muted);font-size:12px}.batch-actions{display:grid;grid-template-columns:1fr 1.4fr;gap:8px;margin-top:12px;position:sticky;bottom:-14px;background:linear-gradient(180deg,transparent,var(--panel) 28%);padding:18px 0 14px}
  ...
  function fillSelectors(){
    ...
    const used=new Set(drafts.map(x=>x.exerciseId));
  ...
  function onInput(e){
    ...
    const ex=e.target.closest('[data-ex-index]'), row=e.target.closest('[data-set-index]'); if(!ex||!row) return;
    const d=drafts[+ex.dataset.exIndex], s=d?.sets?.[+row.dataset.setIndex]; if(s) s[field]=e.target.value;
  }
  ...
  function save(){
    ...
    for(const d of drafts){
      d.sets.forEach((s,i)=>{
        const any=String(s.weight).trim()||String(s.reps).trim()||String(s.rir).trim();
        if(any&&!(+s.reps>0)) valid.push({error:`${d.exerciseName} 第 ${i+1} 组没有填写次数`});
      });
    }
    const err=valid.find(x=>x.error); if(err)return toast(err.error);
    const rows=[]; const now=time();
    drafts.forEach(d=>{
      const entered=d.sets.filter(s=>+s.reps>0); if(!entered.length) return;
